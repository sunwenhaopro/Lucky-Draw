...
@Slf4j
@Component
public class DrawNoticeProducer {
    @Resource
    private  RocketMQTemplate rocketMQTemplate;
    @Resource
    private IRecordServer iRecordServer;
    public  Boolean send(DrawResultVO resultVO) {
        final var activityDrawMessage = new LuckyDrawMessage()
                .setUuid(IdUtil.simpleUUID())
                .setBody(JSON.toJSONString(resultVO));
        Message<LuckyDrawMessage> message = MessageBuilder
                .withPayload(activityDrawMessage)
                .build();
        SendResult sendResult = rocketMQTemplate.syncSend("draw-notice-topic", message);
